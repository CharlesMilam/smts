...
class Custom_Category_Widget extends WP_Widget {
	...
	function Custom_Category_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_categories', 'description' => esc_html__("A list or dropdown of a single category's descendants.", 'qualifire') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 150, 'height' => 350, 'id_base' => 'custom-category-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'custom-category-widget', esc_html__('QualiFire: Custom Category', 'qualifire'), $widget_ops, $control_ops );
	}
...
<?php		    endif;
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__('Categories', 'qualifire'), 'category_id' => '', 'show_as_dropdown' => false, 'show_post_counts' => false, 'show_hierarchy' => false );
		...
		<!-- Show Categories -->
		<p>
			<label for="<?php echo $this->get_field_id( 'category_id' ); ?>"><?php esc_html_e('Category to be displayed:', 'qualifire'); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_as_dropdown' ); ?>">
			    ...
			    <?php esc_html_e('Show as dropdown', 'qualifire'); ?>
			</label><br />
			...
			<label for="<?php echo $this->get_field_id( 'show_post_counts' ); ?>">
			    ...
			    <?php esc_html_e('Show post counts', 'qualifire'); ?>
			</label><br />
			...
			<label for="<?php echo $this->get_field_id( 'show_hierarchy' ); ?>">
			    ...
			    <?php esc_html_e('Show hierarchy', 'qualifire'); ?>
			</label>
		</p>
