...
class Subpages_Widget extends WP_Widget {
	...
	function Subpages_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_subpages', 'description' => esc_html__('Display subpages if present only.', 'qualifire') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'subpages-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'subpages-widget', esc_html__('QualiFire: Subpages', 'qualifire'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( $curr_page_parent )
		    $children = subnav_pages_without_title_attribute("title_li=&sort_column=menu_order&child_of=".$curr_page_parent."&echo=0");
		else
		    $children = subnav_pages_without_title_attribute("title_li=&sort_column=menu_order&child_of=".$page_id."&echo=0");
...
<?php		    /* After widget (defined by themes). */
	...
	function form( $instance ) {
		...
		<p>
		    ...
		    <br />
		    <?php esc_html_e('Leave the Title field blank if you would like to display the parent page Title instead.', 'qualifire'); ?>
		</p>
