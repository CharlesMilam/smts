...
foreach( $c1_slides_array as $slide_row_number ) {
    $transition_types_array[] = $qualifire_options['c1_transition_type_'.$slide_row_number];
}
$transition_types_csv = implode(',', $transition_types_array);

$result = array(
	fx		=> $transition_types_csv,
	speed		=> $qualifire_options['c1_speed'],
	timeout		=> $qualifire_options['c1_timeout'],
	sync		=> ( $qualifire_options['c1_sync'] ) ? 1 : 0
);
