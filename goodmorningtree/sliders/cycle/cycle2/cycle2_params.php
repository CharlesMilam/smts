...
foreach( $c2_slides_array as $slide_row_number ) {
    $transition_types_array[] = $qualifire_options['c2_transition_type_'.$slide_row_number];
}
$transition_types_csv = implode(',', $transition_types_array);

$result = array(
	fx		=> $transition_types_csv,
	speed		=> $qualifire_options['c2_speed'],
	timeout		=> $qualifire_options['c2_timeout'],
	sync		=> ( $qualifire_options['c2_sync'] ) ? 1 : 0,
	texttrans	=> ( $qualifire_options['c2_text_transition_on'] ) ? 1 : 0
);
