...
function simple_breadcrumb(){
	...
	global $post;
	echo '<p class="breadcrumbs"><span class="breadcrumb_info">'.esc_attr__('You are here:', 'qualifire').'</span> <a href="'.get_bloginfo('wpurl').'">';
		//bloginfo('name');
...
function simple_breadcrumb_case($der_post){
	...
	if(is_author()){
		...
		return esc_html__('Author: ', 'qualifire').$curauth->nickname;
	}
	if(is_tag()){ return esc_html__('Tag: ', 'qualifire').single_tag_title('',FALSE); }
	
	if(is_404()){ return esc_html__('404 - Page not Found', 'qualifire'); }
	
	if(is_search()){ return esc_html__('Search', 'qualifire'); }
